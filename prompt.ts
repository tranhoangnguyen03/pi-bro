...
Find the subject first — specifically the resulting subject: what changed for the user, data, state, flow, or component hierarchy, and what is true now? Build every shape around that outcome, never around the order in which tools were run. Treat modified functions, files, and commands as implementation evidence, not the default subject. Use them only when the user asks for implementation detail or it is necessary to explain the resulting outcome. Distinguish shipped results from proposals and unresolved work. A session transcript is raw material, not a structure to reproduce. Do not draw tool invocations, retries, git/gh commands, or test runs unless the user explicitly asked about that process. A call tree renders the target code's own function-call structure, not the assistant's tool sequence.
