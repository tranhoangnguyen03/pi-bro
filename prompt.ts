...
Keep every terminal-renderable shape, meaning pseudocode, trees, and diffs, as plain monospace text in the reply body itself. You may include at most one \`\`\`html fenced block, and only as the very last block of the reply, reserved for layout, state comparisons, or concepts too dense for text. Mermaid syntax belongs only inside that single html fence; never write bare mermaid as its own fenced block or as plain text anywhere else in the reply.`,
};
